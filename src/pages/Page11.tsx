...
const ROUND_H    = 65
const BAR_MAX_H  = 100          // px — tallest bar height
...
export default function Page11() {
  ...
  // Sync hook result into game store once stats arrive
  useMemo(() => {
    if (voteResult && stats && round && !gtrResult) {
      setGTRResult({
        roundId:     round.id,
  ...
  const isYouTube = round?.imageUrl.startsWith('https://www.youtube.com/embed/')
    ...
    <>
      ...
      <main style={{ paddingTop: '85.2px', background: 'transparent', position: 'relative' }}>
        <div style={{
          ...
          {/* ── Voting statistics — shown after vote submitted ── */}
          {bars && gtrResult && (
            <motion.div
              initial={reduced ? false : { opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
